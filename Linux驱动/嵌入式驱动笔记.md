...
> 考虑框架设计理念，在进行接口提供时，需要做到上层接口实现于底层硬件解耦合，即需要将驱动层进行封装，实现出控制接口，在实现给上层的接口时使用。例如：拿led举例，led有打开、关闭、翻转、闪烁等，将这些功能再封装一层，后续底层硬件更换了，对上的接口就不需要改变，只需改底层的驱动。一旦硬件相关的东西和上层接口耦合，后续修改将会十分混乱与麻烦
