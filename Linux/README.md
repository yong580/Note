...
- 一款可以高度自定义的终端工具，可以方便补全命令和进行错误命令显示

- 配置文件尽量做到有层级化，不过配置的东西不多就一个单独的.zshrc就可以了，多文件就是使用在.zshrc下source其他的.zsh文件

## fzf模糊查找文件工具

- 配合ranger文件管理器可以实现在浏览文件夹时进行模糊查找

- 快捷键\<C-r\>能进行模糊查找历史命令

- 常用快捷键
  
  - `ctrl+t`将选择的文件名粘贴到命令行中
  
  - `ctrl+r`模糊查找历史命令，输入到命令行中
...
- 和coc的选择list一样使用`ctrl+j和k`，左右移动为h和l

- 使用`g+其他`可以快速跳转到某个目录,在`~/.config/ranger/rc.conf`文件中进行配置，其他操作也可以看这个文件进行掌握

- 很多文件和文件夹的操作后续需要频繁使用终端时再进行补充，具体可以看`~/.config/ranger/rc.conf`文件
...
- 静态库制作步骤：先生成二进制文件（一系列.o文件），然后使用命令`ar  rsc lib库名.a  +一些列.o文件路径`
...
- 解决方法是提供动态库链接的目录路径：LD_LIBRARY_PATH=动态库路径（==最好使用绝对路径==）
  ...
  1. 修改/etc/ld.so.conf   使用sudo vim /etc/ls.so.conf，然后把动态库的绝对路径粘贴进去（里面会使用include包含其他的.config文件，可以新建一个属于自己动态库路径的文件，方便后续删库跑路）
