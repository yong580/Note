...
- 单片机的栈空间设置一般不会太大，不要在定义很大的数组，会出现栈溢出，会触发hardfault。
...
- 中断处理函数注意点：
  ...
  - 中断处理函数的返回值和形参：
    ...
    中断处理函数不能有返回值和形参，因为中断处理函数都是硬件调用（或者叫触发），没有程序给它传递参数，也没有程序接收它的返回值，其参数的传递通过全局变量的方式。但是要注意，如果在中断服务函数中改变了供其他函数检测的全局变量的值，要使用volatile关键字定义该全局变量。因为主程序可能将该变量读取到寄存器中，以后每次只使用寄存器中的变量副本，这时候如果不使用volatile关键字，会导致中断服务函数中修改该变量的操作被短路。
...
- 注意点：内外括号
- 使用好宏可以提高代码的可读性和扩展性
- 宏函数和普通函数的区别，宏函数不是函数调用，是直接替换，可以减少函数调用的开销，但是增加了代码量，占用内存。

## 3、智能指针

- 内存泄露是指，在堆上申请了内存空间，但使用完毕后没有进行内存释放（是在析构中判断计数值是否为0，为0就释放这块内存）

- 野指针是指，在堆上申请了内存空间后，使用完毕后释放，但是指针未赋空，该指针成为野指针，解引用会访问到未知的内存空间，危害系统的安全。在没有使用shared_ptr的情况下，多个地方使用到同一块内存，如果某一块进行释放，另一个地方还继续使用，原本指向那块内存的指针即为野指针，同样的危害。

- shared_ptr
  
  动态分配内存，只要还有shared_ptr指向的内存块，那块内存就不会被释放，有一个引用计数进行统计，避免有其他对象在使用该内存时却被其他位置的程序给释放了，实现了安全的底层数据共享
  ...
  - 创建智能指针
    ...
    shared_ptr<T> sp = make_shared<T>(arg);arg是根据T类给的构造函数参数列表初始化
    
    auto sp = make_shared<T>(arg)
    ```
    
    ​
  
  - 引用计数：表示有多少个shared_ptr共享一块内存，计数为0时将这块内存释放
    
    ```c++
    //返回当前地址有多少shared_ptr调用,其实就是看智能指针被拷贝的多少次，存在多少个，直到引用计数为0时再释放
    use_count()
    ...
    ```c++
    //若该shared_ptr是唯一的指向该区域的shared指针，返回true，否则返回false
    unique()
    //设置当前共享指针为nullptr，若是最后一个，则会释放内存
    reset()
    ```
    
    ​
  
  - 应用场景：当需要共享底层数据，防止一个地方还在使用另一个地方就已经释放了，导致错误，此时使用分配动态内存的方式，配合上智能指针方便管理。

- unique_ptr
  
  - 最好配合std::make_unique来初始化unique_ptr
  
  - 此指针的特性跟他的名字一样具有唯一性，即无法让两个以上unique指针指向相同的内存地址。
  
  - ```c++
    //返回unique的原生指针,即unique_ptr指向的普通指针
    get() 
    ```

## 4、可重入和不可重入函数

**重入即表示重复进入**，首先它意味着这个函数可以被中断，其次意味着它除了使用自己栈上的变量以外不依赖于任何环境（包括 static），这样的函数就是purecode（纯代码）可重入，可以允许有该函数的多个副本在运行，由于它们使用的是分离的栈，所以不会互相干扰。如果确实需要访问全局变量（包括 static），一定要注意实施互斥手段。可重入函数在并行运行环境中非常重要，但是一般要为访问全局变量付出一些性能代价。
...
- 数据段（data）
  ...
  - **全局存储区（静态存储区）：**
    ...
    全局变量和[静态变量](https://so.csdn.net/so/search?q=%E9%9D%99%E6%80%81%E5%8F%98%E9%87%8F&spm=1001.2101.3001.7020)的存储是放在一块的，初始化的全局变量和静态变量在一块区域， 程序结束后有系统释放。
  
  - **常量存储区：**
    
    这是一块比较特殊的存储区，他们里面存放的是常量(字符串常量和const全局变量，const局部变量存储在栈区)，不允许修改

- 未初始化数据（bss）
  
  存入的是全局未初始化变量和未初始化静态变量。未初始化数据区的数据在程序开始执行之前被内核初始化为0或者空（NULL）。（bss）。
...
- 栈区（stack）
  ...
  存放局部变量、函数参数和返回值、const局部变量

对于静态变量(static)和常量(const)的理解

①static无论是[全局变量](https://so.csdn.net/so/search?q=%E5%85%A8%E5%B1%80%E5%8F%98%E9%87%8F&spm=1001.2101.3001.7020)还是局部变量都存储在全局/静态区域，在编译期就为其分配内存，在程序结束时释放。

②const全局变量存储在**只读数据段**，编译期最初将其保存在符号表中，第一次使用时为其分配内存，在程序结束时释放。

③全局变量存储在全局/静态区域，在编译期为其分配内存，在程序结束时释放，

④局部变量存储在**栈**中，代码块结束时释放，例如：val_h、val_i。

注：当全局变量和静态局部变量未赋初值时，系统自动置为0。(存在与bss段内)

## 6、内存泄漏与检测工具

- valgrind
  - 使用valgrind加上参数运行程序，查看运行结果进行分析